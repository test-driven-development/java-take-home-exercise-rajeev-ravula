...
public class CommaDelimeterParser implements FileParser {

	private static final String COMMA_DELIMETER_PATTERN = "^([\\d\\s]+)([a-zA-Z\\s]+)\\,([\\s\\d\\.\\d|\\d]+)\\,{0,1}([a-zA-Z\\s]+){0,1}";
	...
	LineItem parseLineItem(String lineItemEntry) {
		...
		Pattern p = Pattern.compile(COMMA_DELIMETER_PATTERN);
		...
		if (matcher.matches()) {
			...
			String imported = matcher.group(4) != null ? matcher.group(4).trim() : null;
